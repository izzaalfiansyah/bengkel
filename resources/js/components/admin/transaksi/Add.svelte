<script>
	...
	let store = (e) => {
		...
		http.post('api/transaksi?id_service=' + id_service, request).then(res => {
			if (res.error) {
				...
				if (id_service) {
					navigate('/admin/service/' + id_service + '/detail.html');
				} else {
	...
	let getService = () => {
		http.get('api/service/' + id_service).then(res => { 
			data_service = res.data;
			request.id_pelanggan = res.data.pelanggan.id;
		});
	}

	function getApp() {
		http.get('api/setting').then(res => {
			request.ppn = res.data.ppn
		})
	}
...
<main>
	<Header>Tambah {id_service ? 'Transaksi Service' : 'Transaksi'}</Header>
	<hr>
	<Header>Produk</Header>
	<div class="row">
		<div class="col-md-4">
			<form on:submit={addKeranjangProduk} class="w-100">
				<Card>
					...
					<label for="" style="margin-top: 8px;">Qty</label>
		...
		<div class="col-md-4">
			<form on:submit={addKeranjangJasa}>
				<Card>
					...
					<label for="" style="margin-top: 8px;">Harga</label>
	...
	<Card>
		<div class="row">
			...
			<div class="col-6" align="right">
				<button class="btn btn-primary" on:click="{() => modal.open('#bayar')}">Buat</button>
			</div>
		</div>
	</Card>
	...
	<form on:submit={store}>
		<Modal id="bayar" title="Konfirmasi Pembayaran">
			...
			{:else}
				<select bind:value={request.id_pelanggan} class="form-control">
					<option value="">-</option>
					{#each data_pelanggan as item}
						<option value={item.id}>{item.nama}</option>
					{/each}
